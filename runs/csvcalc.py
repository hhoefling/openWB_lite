    ...
    if (i >= 29) and (i <= 58):
        try:
            ...
            if (
                ...
                and (deltapv >= 0) and (deltapv < 12500) and (deltaspeicherladung >= 0)
                and (deltaspeicherladung < 12500) and (deltaspeicherentladung >= 0)
                and (deltaspeicherentladung < 12500)
            ):
                pass
            else:
                raise Exception("error ratio calc")
            if (
                (i == 29) or (i == 32) or (i == 35) or (i == 38) or (i == 41) or (i == 44) or (i == 47) or (i == 50)
                or (i == 53) or (i == 56)
            ):
            ...
            if (gesamtv > 0):
                try:
                    ...
                    textcol = ''
                print('%s i-err(R) %s:%s c %2d(%s) sum %.3f act %.3f prev %.3f gesamtv %.3f' % (
    ...
    if (newvalue > oldvalue) and (deltarund < 12500) and (oldvalue > 0):
        ...
        if (deltarund > 0):
            try:
                ...
                textcol = ''
            print('%s i-err %s:%s c %2d(%s) sum %.3f act %.3f prev %.3f' % (
                getTime(), datestring, str(row[0]), i, textcol, sumcsv[i], newvalue, oldvalue)
            )
...
def checkyear(calcyear):
    ...
    if ((complastdate != lastdate) or (complastzeit != lastzeit)) and (ifile == 1):
        print('%s checkyear jjjj %s neue Daten alt %s:%s neu %s:%s' % (
            getTime(), calcyear, complastdate, complastzeit, lastdate, lastzeit)
        )
        return 0
...
def checkmonth(jjjjmm):
    ...
    if (dfile == 1) and (validdata == 1) and (ifile == 0):
        ...
        return 0
    if ((complastdate != lastdate) or (complastzeit != lastzeit)) and (ifile == 1):
        print('%s checkmm jjjjmm %s neue Daten alt %s:%s neu %s:%s' % (
            getTime(), jjjjmm, complastdate, complastzeit, lastdate, lastzeit)
        )
        return 0
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--partial', type=str, required=True,
                        help='destination folder for partial logfiles (actual month), should be in ramdisk')
    parser.add_argument('-m', '--mode', type=str, required=True, choices=["M", "A"],
                        help='mode for calculation, M = month, A = all starting 2018/01')
    parser.add_argument('-d', '--date', type=int, required=False, default=time.strftime("%Y%m"),
                        help='in mode M: month to calculate in format YYYYMM, defaults to current month')
