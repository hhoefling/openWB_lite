...
def selmonth(jjjjmm):
    ...
    try:
        ...
        print('%s error %s inhalt %s' % (getTime(), jjjjmm, str(e)))
    if (ifile == 0):
        print('%s summenfile %s nicht gefunden %s' % (getTime(), jjjjmm, file_cvsfinps))
        return
    # header machen
    ...
    try:
        ...
        print('%s error1 %s inhalt %s' % (getTime(), jjjjmm, str(e)))
    if (ifile == 0):
        print('%s datenfile %s nicht gefunden %s' % (getTime(), jjjjmm, file_cvsfinp))
    return
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--partial', type=str, required=True,
                        help='partial logfiles (actual month), should be in ramdisk as input')
    parser.add_argument('-d', '--date', type=int, required=False, default=time.strftime("%Y%m"),
                        help='month to select in format YYYYMM, defaults to current month')
