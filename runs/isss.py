...
def read_meter():
    ...
    if llmeterconfiglp1 == 0:
        ...
        try:
            ...
            log_debug(2, "Erkenne verbauten Zaehler. voltage:" + str(voltage))
            if int(voltage) > 20:
                llmeterconfiglp1 = 105
                sdmid = 105
                log_debug(2, "SDM Zaehler erkannt")
        except Exception:
            log_debug(2, "SDM ist es nicht")
            pass
        # check b23
        try:
            ...
            if int(voltage) > 20:
                ...
                log_debug(2, "B23 ist es nicht, keine spanung erkannt")
        except Exception:
            log_debug(2, "B23 ist es nicht")
            pass
    ...
    try:
        ...
        for key in DeviceValues:
            ...
            if "lp1voltage1" in key:
                ...
                if parentWB != "0":
                    # 1.9 
                    remoteclient.publish("openWB/lp/" + parentCPlp1 + "/Vphase1", payload=str(lp1voltage1),
                                         qos=0, retain=True)
                    # 2,0
                    remoteclient.publish("openWB/set/chargepoint/" + parentCPlp1 + "/get/voltages",
                                         payload="[" + str(lp1voltage1) + "," + str(lp1voltage2) + ","
                                         + str(lp1voltage3) + "]", qos=0, retain=True)
                    remoteclient.loop(timeout=2.0)
            ...
            if "lp1lla2" in key:
                ...
                if parentWB != "0":
                    remoteclient.publish("openWB/lp/" + parentCPlp1 + "/Aphase2", payload=str(lp1lla2),
                                         qos=0, retain=True)
                    remoteclient.loop(timeout=2.0)
            ...
            if "lp1lla3" in key:
                ...
                if parentWB != "0":
                    remoteclient.publish("openWB/lp/" + parentCPlp1 + "/Aphase3", payload=str(lp1lla3),
                                         qos=0, retain=True)
                    remoteclient.loop(timeout=2.0)
            ...
            if "lp1countphasesinuse" in key:
                ...
                if parentWB != "0":
                    # 1.9 
                    remoteclient.publish("openWB/lp/" + parentCPlp1 + "/countPhasesInUse",
            ...
            if "lp1plugstat" in key:
                ...
                if parentWB != "0":
                    # 1.9
                    remoteclient.publish("openWB/lp/" + parentCPlp1 + "/boolPlugStat",
            ...
            if "lp1chargestat" in key:
                ...
                if parentWB != "0":
                    # 1.9 
                    remoteclient.publish("openWB/lp/" + parentCPlp1 + "/boolChargeStat",
            ...
            if lp2installed:
                ...
                if "lp2watt" in key:
                    ...
                    if parentWB != "0":
                        # 1.9 
                        remoteclient.publish("openWB/lp/" + parentCPlp2 + "/W", payload=str(lp2llg),
                                             qos=0, retain=True)
                        # 2,0
                ...
                if "lp2voltage1" in key:
                    ...
                    if parentWB != "0":
                        ...
                        remoteclient.publish("openWB/set/chargepoint/" + parentCPlp2 + "/get/voltages",
                                             payload="[" + str(lp2voltage1) + "," + str(lp2voltage2) + ","
                                             + str(lp2voltage3) + "]", qos=0, retain=True)
                        remoteclient.loop(timeout=2.0)
                ...
                if "lp2lla1" in key:
                    ...
                    if parentWB != "0":
                        ...
                        remoteclient.publish("openWB/set/chargepoint/" + parentCPlp2 + "/get/currents",
                                             payload="[" + str(lp2lla1) + "," + str(lp2lla2) + ","
                                             + str(lp2lla3) + "]", qos=0, retain=True)
                        remoteclient.loop(timeout=2.0)
                ...
                if "lp2plugstat" in key:
                    if DeviceValues[str(key)] != Values["lp2plugstat"]:
                        mclient.publish("openWB/lp/2/boolPlugStat", payload=Values["lp2plugstat"],
                                        qos=0, retain=True)
                        mclient.loop(timeout=2.0)
                    ...
                    if parentWB != "0":
                        # 1.9 
                        remoteclient.publish("openWB/lp/" + parentCPlp2 + "/boolPlugStat",
                ...
                if "rfidtag2" in key:
                    if DeviceValues[str(key)] != str(rfidtag2):
                        mclient.publish("openWB/lp/2/LastScannedRfidTag", payload=str(rfidtag2),
                                        qos=0, retain=True)
                        mclient.loop(timeout=2.0)
