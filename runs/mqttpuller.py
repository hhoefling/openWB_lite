...
def getmodules():
	...
	modules=[]
	if C.readval('evsecon') == 'mqtt_puller':
		modules.append('ev1')
	if C.readval('ladeleistungmodul') == 'mqtt_puller':
		modules.append('evll1')
	if C.readval('socmodul') == 'soc_mqttpuller':
		modules.append('evsoc1')

	if C.readval('lastmanagement') == "1":
		if C.readval('evsecons1') == 'mqtt_puller':
			modules.append('ev2')
		if C.readval('ladeleistungs1modul') == 'mqtt_puller':
			modules.append('evll2')
		if C.readval('socmodul') == 'soc_mqttpuller':
			modules.append('evsoc2')

	if C.readval('wattbezugmodul') == 'bezug_mqttpuller':
		modules.append('evu')  
	if C.readval('pvwattmodul') == 'wr_mqttpuller':
		modules.append('pv')   
	if C.readval('speichermodul') == 'speicher_mqttpuller':
		modules.append('bat')  
