...
evsedintest() {
	...
	if [[ $evsedintestlp1 == "ausstehend" ]]; then
		if [ $evsecon == "modbusevse" ] || [ $evsecon == "masterethframer" ]; then
			if [[ $evsecon == "modbusevse" ]]; then 
				...
				sleep 1
				sudo python3 runs/evsewritembusdev.py $modbusevsesource $modbusevseid 1000 9
				sleep 1
				...
				sleep 1
				sudo python3 runs/evsewritembusdev.py $modbusevsesource $modbusevseid 1000 0
				sleep 1
			fi
			if [[ $evsecon == "masterethframer" ]]; then
				sudo python3 runs/evsewritembusethframerdev.py 192.168.193.18 1 1000 9
				sleep 1
				evsedinstat=$(sudo python3 runs/readmodbusethframer.py 192.168.193.18 1 1000 1)
				if [[ $evsedinstat == "[9]" ]]; then
					openwbDebugLog "MAIN" 0 "EVSE LP1 Prüfung erfolgreich"
					echo "erfolgreich" > ramdisk/evsedintestlp1
				else
				...
				sleep 1
				sudo python3 runs/evsewritembusethframerdev.py 192.168.193.18 1 1000 0
				sleep 1
			fi
		else
	...
	if [[ $evsedintestlp2 == "ausstehend" ]]; then
		if [[ $evsecons1 == "modbusevse" ]]; then
			...
			sleep 1
			sudo python3 runs/evsewritembusdev.py $evsesources1 $evseids1 1000 9
			sleep 1
			...
			sleep 1
			sudo python3 runs/evsewritembusdev.py $evsesources1 $evseids1 1000 0
			sleep 1
		else
	...
	if [[ $evsedintestlp3 == "ausstehend" ]]; then
		if [[ $evsecons2 == "modbusevse" ]]; then
			...
			sleep 1
			sudo python3 runs/evsewritembusdev.py $evsesources2 $evseids2 1000 9
			sleep 1
			...
			sleep 1
			sudo python3 runs/evsewritembusdev.py $evsesources2 $evseids2 1000 0
			sleep 1
		else
	...
	if [[ $evseausgelesen == "0" ]]; then
		if [ $evsecon == "modbusevse" ] || [ $evsecon == "masterethframer" ]; then
			if [[ $evsecon == "modbusevse" ]]; then 
				...
				sleep 1
                openwbDebugLog "MAIN" 0 "EVSE LP1 EVSE gelesen 2000:$evselp12000 2007:$evselp12007"  
			fi
			if [[ $evsecon == "masterethframer" ]]; then
				sleep 1
				evselp12000=$(sudo python3 runs/readmodbusethframer.py 192.168.193.18 1 2000 1)
				echo $evselp12000 > /var/www/html/openWB/ramdisk/progevsedinlp12000
				sleep 1
				evselp12007=$(sudo python3 runs/readmodbusethframer.py 192.168.193.18 1 2007 1)
				echo $evselp12007 > /var/www/html/openWB/ramdisk/progevsedinlp12007
				sleep 1
                openwbDebugLog "MAIN" 0 "EVSE LP1 EVSE gelesen 2000:$evselp12000 2007:$evselp12007"  
			fi
	...
	if [[ $progevselp1 == "1" ]]; then
		if [ $evsecon == "modbusevse" ] || [ $evsecon == "masterethframer" ]; then
			if [[ $evsecon == "modbusevse" ]]; then 
				...
				sudo python3 runs/evsewritembusdev.py $modbusevsesource $modbusevseid 2000 $lp12000
				sleep 1
				read lp12007 <ramdisk/progevsedinlp12007
				sudo python3 runs/evsewritembusdev.py $modbusevsesource $modbusevseid 2007 $lp12007
                openwbDebugLog "MAIN" 0 "EVSE LP1 EVSE writen 2000:$lp12000 2007:$lp12007"  
			fi
			if [[ $evsecon == "masterethframer" ]]; then
				read lp12000 <ramdisk/progevsedinlp12000
				sudo python3 runs/evsewritembusethframerdev.py 192.168.193.18 1 2000 $lp12000
				sleep 1
				read lp12007 <ramdisk/progevsedinlp12007
				sudo python3 runs/evsewritembusethframerdev.py 192.168.193.18 1 2007 $lp12007
			fi
		fi
	...
	if [[ $progevselp2 == "1" ]]; then
		if [[ $evsecons1 == "modbusevse" ]]; then 
			...
			sudo python3 runs/evsewritembusdev.py $evsesources1 $evseids1 2000 $lp22000
			sleep 1
			read lp22007 <ramdisk/progevsedinlp22007
            sudo python3 runs/evsewritembusdev.py $evsesources1 $evseids1 2007 $lp22007
		fi
...
evsemodbuscheck() {
	if [[ $evsecon == "modbusevse" ]]; then
		...
		if [[ $evsedinstat == "$llalt" ]]; then
			openwbDebugLog "MAIN" 1 "evsemodbuscheck LP1 Modbus $llalt korrekt"
		else
			openwbDebugLog "MAIN" 1 "evsemodbuscheck LP1 Modbus $llalt nicht korrekt"
			sudo python3 runs/evsewritembusdev.py $modbusevsesource $modbusevseid 1000 $llalt
		fi
	fi
	if (( lastmanagement == 1 )); then 
		if [[ $evsecons1 == "modbusevse" ]]; then
			...
			if [[ $evsedinstat == "$llalts1" ]]; then
				openwbDebugLog "MAIN" 1 "evsemodbuscheck LP2 Modbus $llalts1 korrekt"
			else
				openwbDebugLog "MAIN" 1 "evsemodbuscheck LP2 Modbus $llalts1 nichtkorrekt"
				sudo python3 runs/evsewritembusdev.py $evsesources1 $evseids1 1000 $llalts1
			fi
		fi
		if (( lastmanagements2 == 1 )); then
			if [[ $evsecons2 == "modbusevse" ]]; then
				...
				if [[ $evsedinstat == "$llalts2" ]]; then
					openwbDebugLog "MAIN" 1 "evsemodbuscheck LP3 Modbus $llalts2 korrekt"
				else
					openwbDebugLog "MAIN" 1 "evsemodbuscheck LP3 Modbus $llalts2 nicht korrekt"
					sudo python3 runs/evsewritembusdev.py $evsesources2 $evseids2 1000 $llalts2
				fi
			fi
		fi
	fi
