...
def getCookies():
    ...
    url = parameters.getParameter('baseUrl') + '/api/v1/user/oauth2/authorize?response_type=code&state=test&client_id=' + parameters.getParameter('clientId') + '&redirect_uri=' + parameters.getParameter('baseUrl') + '/api/v1/user/oauth2/redirect'
...
def setLanguage(cookies):
    ...
    try:
        response = kiahttp.postHTTP(url = url, data = data, headers = headers, cookies = cookies, timeout = parameters.getParameter('reqTimeout'))
    except:
        raise
...
def getAuthCode(cookies):
    ...
    try:
        response = kiahttp.getHTTP(url = url, headers = headers, cookies = cookies, timeout = parameters.getParameter('reqTimeout'))
    except:
        raise 
    ...
    data = urlparse.urlencode({'username': parameters.getParameter('accountName'), 'password': parameters.getParameter('accountPassword'), 'credentialId': '', 'rememberMe': 'on'})
    headers = {
        'Content-type': 'application/x-www-form-urlencoded',
    ...
    try:
        response = kiahttp.postHTTP(url = url, data = data, headers = headers, cookies = cookiesForm, timeout = parameters.getParameter('reqTimeout'))
    ...
    try:
        response = kiahttp.postHTTP(url = url, data = data, headers = headers, cookies = cookies, timeout = parameters.getParameter('reqTimeout'), allow_redirects = False)
        responseDict = json.loads(response)
...
def getAuthToken(authCode):
    ...
    url = parameters.getParameter('baseUrl') + '/api/v1/user/oauth2/token'
    data = 'grant_type=authorization_code&redirect_uri=' + parameters.getParameter('baseUrl') + '%2Fapi%2Fv1%2Fuser%2Foauth2%2Fredirect&code=' + authCode
    headers = {
        'Authorization': parameters.getParameter('basicToken'),
        'Content-type': 'application/x-www-form-urlencoded',
        'Content-Length': str(len(data)),
