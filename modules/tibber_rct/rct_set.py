#!/usr/bin/python3
#
# RCT_tibber submodule zur steuerung des Wechselrichters
#
...
def readold(sock):
    ...
    rctw.dbglog("readold: %s >> %s >> %s >> %s [%s] W" % (istP5,istP7,istPsoc,istP97, istWatt))
    rctw.dbglog("readold: DischargeMax: %s" % (istmaxDischarge))
...
def resetwatt(sock):
    ...
    if istP5 != 0.05:
        ...
        print("Korrigiere %s [%s] => [%s] Written" % (oP5.name, istP5, nowP5))
        done=1
    else:
        print("Value of %s already [%s] ok" % (oP5.name ,istP5))

    if istP7 != 0.07:
        rctw.write(sock, oP7.id, 0.07)
        time.sleep(1)
        nowP7 = oread(sock, oP7)
        print("Korrigiere %s [%s] => [%s] Written" % (oP7.name, istP7, nowP7))
        done=1
    else:
        print("Value of %s already [%s] ok" % (oP7.name ,istP7))

    if istP97 != 0.97:
        rctw.write(sock, oP97.id, 0.97)
        time.sleep(1)
        nowP97 = oread(sock, oP97)
        print("Korrigiere %s [%s] => [%s] Written" % (oP97.name, istP97, nowP97))
        done=1
    else:
        rctw.dbglog("Value of %s already [%s] ok" % (oP97.name ,istP97))

    if istWatt != 100.0:
        rctw.write(sock, oWatt.id, 100.0)
        time.sleep(1)
        nowWatt = oread(sock, oWatt)
        print("Korrigiere %s [%s] => [%s] Written" % (oWatt.name, istWatt, nowWatt))
        done=1
    else:
        print("Value of %s already [%s] ok" % (oWatt.name ,istWatt))
...
def resetcurrent(sock):
    ...
    if istmaxDischarge != 20.0:
        ...
        print("Korrigiere %s [%s] => [%s] Written" % (omaxDischarge.name, istmaxDischarge, nowmaxDischarge))
        done=1
    else:
        rctw.dbglog("%s allread [%s] ok" % (omaxDischarge.name, istmaxDischarge) )
    if( done>0):
        rctw.dbglog('Reload values')
        readold(sock)
    ...
    if istPsoc >= istP97:
        ...
        if istPsoc > newP7:
            ...
            if istP7 != newP7:
                ...
                rctw.dbglog(oP7.name + ' ' + str(newP7) + ' Written')
                print('RCT_SET ', oP7.name, ' ', str(newP7), ' Written')
            else:
            ...
            if istP5 != newP5:
                ...
                rctw.dbglog(oP5.name + ' ' + str(newP5) + ' Written')
                print('RCT_SET ' + oP5.name + ' ' + str(newP5) + ' Written')
            else:
            ...
            if newWatt != istWatt:
                ...
                rctw.dbglog(oWatt.name + ' ' + str(newWatt) + ' Written')
                print('RCT_SET ' + oWatt.name + ' ' + str(newWatt) + ' Written')
            else:
...
def xstopdrain(sock):
    ...
    if( newP7 < 0.07):
        ...
        print('RCT_SET Set ' + oP7.name + ' from ' + str(istP7) + ' to ' + str(nowP7))
        rctw.write(sock, oP7.id, newP7)
        time.sleep(1)
        nowP7 = oread(sock, oP7)
...
def drain1A(sock):
    ...
    if istmaxDischarge != sollcurent:
        ...
        time.sleep(1)
        nowmaxDischarge = oread(sock, omaxDischarge)
        rctw.dbglog("%s [%s] => [%s] Written" %(omaxDischarge.name, istmaxDischarge, nowmaxDischarge))
        print("RCT_SET Set %s [%s] => [%s] Written" %(omaxDischarge.name, istmaxDischarge, nowmaxDischarge))
        # Klappt,  achku wird nur noch mit rund 2300W Entladen (1*BatV) 
    else:
...
def drain(sock):
    ...
    if istmaxDischarge != sollcurent:
        ...
        nowmaxDischarge = oread(sock, omaxDischarge)
        rctw.dbglog(" %s [%s] => [%s] Written" % (omaxDischarge.name,istmaxDischarge,nowmaxDischarge))
        print("RCT_SET %s [%s] => [%s] Written" % (omaxDischarge.name,istmaxDischarge,nowmaxDischarge))
