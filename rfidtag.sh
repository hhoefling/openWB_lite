...
function rfid() {
	...
	if [[ $lasttag != "0" ]]; then
		...
		if [ "$lasttag" == "$rfidlp1start1" ] || [ "$lasttag" == "$rfidlp1start2" ] || [ "$lasttag" == "$rfidlp1start3" ] || [ "$lasttag" == "$rfidlp1start4" ] || [ "$lasttag" == "$rfidlp1start5" ]; then
			...
			lp1enabled=1
            echo $lp1enabled > ramdisk/lp1enabled
		...
		if [ "$lasttag" == "$rfidlp2start1" ] || [ "$lasttag" == "$rfidlp2start2" ] || [ "$lasttag" == "$rfidlp2start3" ] || [ "$lasttag" == "$rfidlp2start4" ] || [ "$lasttag" == "$rfidlp2start5" ]; then
			...
			lp2enabled=1
            echo $lp2enabled > ramdisk/lp2enabled
			tagScanInfo="$NowItIs,$lasttag,1"
...
# private
function checkTagValidForSocket() {
	...
	local rfidlist
	read rfidlist <$ramdiskFileForSocket
	...
	do
		if [ "$lasttag" == "$i" ] ; then
			...
			if [ -f $SocketActivationFile ]; then
				...
				local requested
				local active
				read requested <$SocketActivationFile
				read active <ramdisk/socketActivated
...
# NC 
function checkTagValidAndSetStartScanData() {

	local chargePoint=$1
    slavemode=0     # yourCharge mode
	...
	do
		if [ "$lasttag" == "$i" ] ; then
            ...
            echo "1" > "ramdisk/lp${chargePoint}enabled"
