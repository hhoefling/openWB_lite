...
var thevalues = [
	["openWB/system/YearGraphDatan1", "#"],
	["openWB/system/YearGraphDatan2", "#"],
	["openWB/system/YearGraphDatan3", "#"],
	["openWB/system/YearGraphDatan4", "#"],
	["openWB/system/YearGraphDatan5", "#"],
	["openWB/system/YearGraphDatan6", "#"],
	["openWB/system/YearGraphDatan7", "#"],
	["openWB/system/YearGraphDatan8", "#"],
	["openWB/system/YearGraphDatan9", "#"],
	["openWB/system/YearGraphDatan10", "#"],
	["openWB/system/YearGraphDatan11", "#"],
	["openWB/system/YearGraphDatan12", "#"],
...
function handlevar(mqttmsg, mqttpayload, mqtttopic, htmldiv) {
	...
	if ( mqttmsg.match( /^openwb\/system\/yeargraphdatan[1-9][0-9]*$/i ) ) {
		...
		if (index == 4 && initialread == 0 && (mqttpayload != "empty")) {
			graphDataSegments [indexc] = mqttpayload;
			indexc = indexc + 1
		}
		if (index == 5  && initialread == 0 && (mqttpayload != "empty")) {
...
function requestyeargraph() {
	publish(graphDate, "openWB/set/graph/RequestYearGraphv1");
}
...
function loadgraph() {
  ...
  csvDatab.pop();  // discard last row in csvData-array, it was just needed for calculation of daily values
	...
	for ( var columnIndex = 1; columnIndex < csvData[0].length; columnIndex++ ) {
		...
		if (total <= 24) {  // set data total less then 24 kw to disabled
			totalHidden.push(1);
		} else {
			totalHidden.push(0)
		}
...
//	}
	...
	for ( var column = 1; column < csvData[0].length; column++ ) {
		...
		var dataColumn = getCol(csvDatab, column);
	...
	// filter out base on the called .php
	...
	$('#canvas').click (function(evt) {
      ...
      if (callv1 == 1) {
        window.location.href = "logging/monthlyv1.php?date=" + jumpToDate;
      } ;
      if (callv2 == 1) {
        window.location.href = "logging/monthlyv2.php?date=" + jumpToDate;
      } ;
	 	}
	});
