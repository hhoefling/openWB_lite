...
function storeSparklineValue(element, value) {
  ...
  for (index = 0; index < lastSparklineValues.length && !done; index++) {
    if (
      lastSparklineValues[index].chartElement.attr("data-chartName") ==
      element.attr("data-chartName")
    ) {
...
function updateSparklines() {
  ...
  for (index = 0; index < lastSparklineValues.length; index++) {
    ...
    var chartdataarray = chartdata ? chartdata.split(",") : [];
...
function handlevar(mqttmsg, mqttpayload) {
  ...
  if (mqttmsg.match(/^openwb\/evu\//i)) {
    processEvuMessages(mqttmsg, mqttpayload);
  } else if (mqttmsg.match(/^openwb\/global\//i)) {
    processGlobalMessages(mqttmsg, mqttpayload);
  } else if (mqttmsg.match(/^openwb\/housebattery\//i)) {
    processHousebatteryMessages(mqttmsg, mqttpayload);
  } else if (mqttmsg.match(/^openwb\/system\//i)) {
    processSystemMessages(mqttmsg, mqttpayload);
  } else if (mqttmsg.match(/^openwb\/pv\//i)) {
    processPvMessages(mqttmsg, mqttpayload);
  } else if (mqttmsg.match(/^openwb\/lp\//i)) {
    processLpMessages(mqttmsg, mqttpayload);
  } else if (mqttmsg.match(/^openwb\/config\/get\/global\//i)) {
    processGlobalConfigMessages(mqttmsg, mqttpayload);
  } else if (mqttmsg.match(/^openwb\/config\/get\/sofort\/lp\//i)) {
    processSofortConfigMessages(mqttmsg, mqttpayload);
  } else if (mqttmsg.match(/^openwb\/config\/get\/pv\//i)) {
    processPvConfigMessages(mqttmsg, mqttpayload);
  } else if (mqttmsg.match(/^openwb\/config\/get\/display\//i)) {
    processDisplayConfigMessages(mqttmsg, mqttpayload);
  }
} // end handlevar
...
function processDisplayConfigMessages(mqttmsg, mqttpayload) {
  ...
  if (mqttmsg == "openWB/config/get/display/showHouseConsumption") {
    ...
    mqttmsg.match(/^openwb\/config\/get\/display\/chartLp\/[1-9][0-9]*\/max$/i)
  ) {
    var index = getIndex(mqttmsg); // extract number between two / /
    if( index > 3) { return; }
    var chartElement = $(
      ".sparkline[data-chartname=ladepunkt" + index + "llchart]"
    );
    chartElement.attr("data-sparkChartRangeMax", mqttpayload);
  }
}
  ...
  if (mqttmsg == "openWB/config/get/pv/priorityModeEVBattery") {
    ...
    switch (mqttpayload) {
      case "0":
        ...
        $(".priorityEvBatteryIcon")
          .removeClass("fa-car")
          .addClass("fa-car-battery");
        break;
      case "1":
        // ev priority
...
const processGlobalConfigMessages = (mqttmsg, mqttpayload) => {
  if (mqttmsg == "openWB/config/get/global/maxEVSECurrentAllowed") {
    maxKWhAllowed = Number(mqttpayload) * 230 * 3;
    $(".maxKWhAllowed").text(Math.round(parseFloat(maxKWhAllowed / 1000, 10)));
    calcPercent();
  }
};
...
function processEvuMessages(mqttmsg, mqttpayload) {
  ...
  if (mqttmsg == "openWB/evu/W") {
    ...
    if (powerEvuValue >= 1000) {
      powerEvuText = (powerEvuValue / 1000).toLocaleString(undefined, {
...
const calcPercent = () => {
  var progress = ((kWhactualcharged / maxKWhAllowed) * 100).toFixed(0);
  $("#loadingpower").width(progress + "%");
};

let maxKWhAllowed = 4;
let kWhactualcharged = 0;
...
function processGlobalMessages(mqttmsg, mqttpayload) {
  ...
  if (mqttmsg == "openWB/global/WHouseConsumption") {
    ...
    if (powerHouse > 999) {
      powerHouseText = (powerHouse / 1000).toLocaleString(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      });
      unit = " kW";
    ...
    kWhactualcharged = Math.min(powerAllLp, maxKWhAllowed);
    calcPercent();
    powerAllLpText = powerAllLp.toString();
    // if (kWhactualcharged > 999) {
    powerAllLpText = (kWhactualcharged / 1000).toLocaleString(undefined, {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    });
    unit = " kW";
    // }
    var element = $("#gesamtll,.gesamtll-value");
    ...
    const statusIndicator = $("#lm-status");
    const inactiveText = $(".lm-inactive-text");
    if (mqttpayload == "Slave Mode Aktiv, openWB NUR fernsteuerbar") {
      statusIndicator.addClass("active").removeClass("error");
      inactiveText.addClass("hide");
    } else {
      statusIndicator.addClass("error").removeClass("active");
      inactiveText.removeClass("hide");
    }
...
function processHousebatteryMessages(mqttmsg, mqttpayload) {
  ...
  if (mqttmsg == "openWB/housebattery/W") {
    ...
    if (speicherwattValue > 999) {
      speicherwattText = (speicherwatt / 1000).toLocaleString(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      });
      unit = " kW";
...
const getDateTimeFromTimestamp = (timestamp) => {
  var dateObject = new Date(timestamp * 1000); // Unix timestamp to date-object
  var time = "&nbsp;";
  var date = "&nbsp;";
  var year = "&nbsp;";
  ...
  if (dateObject instanceof Date && !isNaN(dateObject.valueOf())) {
    ...
    var dayOfWeek = dateObject.toLocaleDateString("de-DE", {
      weekday: "short",
    });
    date = dayOfWeek + ", " + dd + "." + mm + "." + year;
  }
  return { date, time, dateObject, dd, mm, year };
};
...
function processSystemMessages(mqttmsg, mqttpayload) {
  ...
  if (mqttmsg == "openWB/system/Timestamp") {
    const { date, time } = getDateTimeFromTimestamp(mqttpayload);
    $("#time").text(time);
    $("#date").text(date);
...
function processPvMessages(mqttmsg, mqttpayload) {
  ...
  if (mqttmsg == "openWB/pv/W") {
    ...
    if (pvwatt > 999) {
      pvwattText = (pvwatt / 1000).toLocaleString(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      });
      unit = " kW";
...
const setTexColorForCondition = (element, condition) => {
  if (condition) {
    element.addClass("color-primary").removeClass("text-red");
  } else {
    element.removeClass("color-primary").addClass("text-red");
  }
};

const setDataByClassName = (
  className,
  mqttpayload,
  trueMessage,
  falseMessage
  ...
  const condition = mqttpayload == "1";
  element.text(condition ? trueMessage : falseMessage);
  if (condition) {
    element.addClass("color-primary");
  } else {
    element.removeClass("color-primary");
  }
};
...
function processLpMessages(mqttmsg, mqttpayload) {
  ...
  if (mqttmsg.match(/^openwb\/lp\/[1-9][0-9]*\/w$/i)) {
    ...
    if (actualPower > 999) {
      actualPowerText = (actualPower / 1000).toLocaleString(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      });
      unit = " kW";
    ...
    if (element.length)
      updateDashboardElement(
        element,
        elementChart,
        actualPowerText + unit,
        actualPower
      );
  } else if (mqttmsg.match(/^openwb\/lp\/1*\/AConfigured$/i)) {
    $(".AConfigured").text(mqttpayload);
  } else if (mqttmsg.match(/^openwb\/lp\/1*\/ChargePointEnabled$/i)) {
    setDataByClassName(
      "ChargePointEnabled",
      mqttpayload,
      "freigegeben",
  ...
  } else if (mqttmsg.match(/^openwb\/lp\/1*\/boolPlugStat$/i)) {
    if (mqttpayload == "1") {
      $("#charger").addClass("plugged");
    } else {
      $("#charger").removeClass("plugged");
    }
    setDataByClassName(
      "boolPlugStat",
      mqttpayload,
  ...
  } else if (mqttmsg.match(/^openwb\/lp\/1*\/boolChargeStat$/i)) {
    setDataByClassName("boolChargeStat", mqttpayload, "lädt", "lädt nicht");
  } else if (mqttmsg.match(/^openwb\/lp\/1*\/tagScanInfo$/i)) {
    const [timestamp, chipId, status] = mqttpayload.split(",");

    const { date, time } = getDateTimeFromTimestamp(timestamp);
    $(".rfid-timestamp").text(`${date} ${time}`);
    $(".rfid-chipId").text(chipId);
    let message = "Unbekannt";
    switch (status) {
      case "0":
        message = "Chip ungültig";
        break;
      case "1":
        message = "Auto laden";
        break;
      case "2":
        message = "Steckdose nutzen";
        break;
    }
    $(".rfid-status").text(message);

    setTexColorForCondition($(".rfid-status"), Number(status) > 0);
    if (
      Math.floor(Date.now() / 1000) - timestamp <
        inactivityTimeout / 1000 - 1 &&
      status > 0 &&
  ...
  } else if (mqttmsg.match(/^openwb\/lp\/1*\/countPhasesInUse$/i)) {
    //const index = getIndex(mqttmsg);
    const phases = Number(mqttpayload);
    $(`.status-indicator.status-${index}`)
      .addClass("inactive")
      .removeClass("active");

    if (phases > 0) {
      $("#charger").addClass("active").removeClass("inactive");
    } else {
      $("#charger").removeClass("active").addClass("inactive");
    }
    for (let i = 1; i < phases + 1; i++) {
      $(`.status-indicator.status-${index}-${i}`)
        .addClass("active")
        .removeClass("inactive");
    }
  } else if (mqttmsg.match(/^openwb\/lp\/[1-9][0-9]*\/kWhDailyCharged$/i)) {
    //const index = getIndex(mqttmsg);
    var energyCharged = parseFloat(mqttpayload, 10);
    if (isNaN(energyCharged)) {
      energyCharged = 0;
    }
    if (energyCharged <= 0.3) {
      energyCharged = 0;
    }
    $(".kWhDailyCharged-" + index).text(
      energyCharged.toLocaleString(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }) + " kWh"
    );
    ...
    //const index = getIndex(mqttmsg);
    var kWhCounter = parseFloat(mqttpayload, 10);
    if (isNaN(kWhCounter)) {
      kWhCounter = 0;
    }
    $(".kWhCounter-" + index).text(
      kWhCounter.toLocaleString(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }) + " kWh"
    );
  } else if (
    mqttmsg.match(/^openwb\/lp\/[1-9][0-9]*\/kWhchargedsinceplugged$/i)
  ) {
    ...
    if (!isNaN(consumption) && consumption > 0) {
      ...
      kmCharged =
        kmCharged.toLocaleString(undefined, {
          minimumFractionDigits: 1,
          maximumFractionDigits: 1,
        }) + " km";
    } else {
    ...
    $(".kWhchargedsinceplugged-" + index).text(
      energyCharged.toLocaleString(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }) + " kWh"
    ...
    // console.log("=mqttpayload=", mqttpayload, limit);

    $(".kWhactualcharged-" + index).text(
      actualCharged.toLocaleString(undefined, {
        minimumFractionDigits: 1,
        maximumFractionDigits: 1,
      }) + " kWh"
    );
    ...
    mqttmsg.match(/^openwb\/lp\/[1-9][0-9]*\/boolchargepointconfigured$/i)
  ) {
    ...
    switch (mqttpayload) {
      case "0":
        ...
        element.removeClass(
          "fa-lock fa-lock-open animate-alertPulsation text-red text-green"
        );
      ...
      case "2":
        // add red locked icon
        element.removeClass(
          "fa-lock-open fa-key animate-alertPulsation text-green"
        );
    ...
    mqttmsg.match(/^openwb\/lp\/[1-9][0-9]*\/energyconsumptionper100km$/i)
  ) {
    ...
    mqttmsg.match(/^openwb\/lp\/[1-9][0-9]*\/boolfinishattimechargeactive$/i)
  ) {
    // respective charge point configured
    //var index = getIndex(mqttmsg); // extract number between two / /
    var parent = $('[data-lp="' + index + '"]'); // get parent row element for charge point
