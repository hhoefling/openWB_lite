...
var options = {
	...
	onSuccess: function () {
		...
		topicsToSubscribe.forEach((topic) => { clientsubscribe(topic[0], {qos: 0}); });
		if( iscloud )
		{
			console.log('Subscribe Cloud graph');
			topicsToSubscribeCloud.forEach((topic) => { clientsubscribe(topic[0], {qos: 0}) });
			publish("1",  "openWB/set/graph/RequestLLiveGraph");
		}  		  
		else
		{ 
			console.log('Subscribe local graph');
		  topicsToSubscribeLocal.forEach((topic) => { clientsubscribe(topic[0], {qos: 0} ); });
		}  
